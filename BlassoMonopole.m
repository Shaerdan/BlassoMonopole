...
for ii = 1:GlobalIteration
    ...
    %%%%%%%%%% Update the source intensity by solving a linear lasso with fbs solver %%%%%%%%%%%%%%
    
    IntensityUpdate = 0;
    InitialIntensity = [Solution.Intensity, IntensityUpdate]';
    PhiComponent = ComputePotentialComponent(Solution.Location,Mesh);
    F_L2Norm = L2NormF(Solution.Location,Mesh);
    %     [Solution.Intensity] = FBSSolver(InitialIntensity,Data,FBS.Iteration,PhiComponent,Mesh.ThetaS,Mesh.PsiS,FBS.GradTol,FBS.StepTol,FBS.Tau,Lambda,FBS.DisplayFrequency,F_L2Norm);
    %     Solution.Intensity = Solution.Intensity';
    
    SourceNumUpdate = ii;
    S_max = 5;
    LowerBoundLinear = -S_max*ones(1,SourceNumUpdate)';    % Lower bound = -S_max* RealIntensity
    UpperBoundLinear = S_max*ones(1,SourceNumUpdate)';     % Upper bound =  S_max* RealIntensity
    problem = createOptimProblem('fmincon','x0',InitialIntensity,'ub',UpperBoundLinear,'lb',LowerBoundLinear, ...
        'objective',@(X) ObjectiveFuncLinearLasso(X,Data.Measurement,PhiComponent,Mesh,F_L2Norm,Lambda),...
        'options',Opts);
    ...
    %
    %%%%%%%%%%% Solve the non-convex minimization to get x*,S*
    InitialSolution = [Solution.Intensity,Solution.Location]';
    %  %%%%%% solver2: lbfgsb package used by Peyrr for sparse sensing:
    
    %  tic
    %     options1 = struct('x0',InitialSolution,'m',20,'factr',1e0,'pgtol',1e-30,'maxIts',10000,'maxTotalIts',500000,'printEvery',1);
    %     S_max = 2;
    %     SourceNumUpdate = length(a);
    %     lb1 = [-S_max*ones(1,SourceNumUpdate), repmat(0.1,1,SourceNumUpdate), zeros(1,SourceNumUpdate),zeros(1,SourceNumUpdate)]';
    %     ub1 = [S_max*ones(1,SourceNumUpdate),  repmat(0.9,1,SourceNumUpdate), repmat(pi,1,SourceNumUpdate),repmat(2*pi,1,SourceNumUpdate)]';
    %     [SolutionArgmin,f_val2,info2] = lbfgsb( @(X) ObjectiveFuncNonLinearLasso(X,Data,lambda, Mesh, F_L2Norm),...
    %                         lb1, ub1, options1 );
    %  toc
    
    %     InitialSolution=rand(4*SourceNum,1);
    SourceNumUpdate = length(Solution.Intensity);
    LowerBoundNonLinear = [-S_max*ones(1,SourceNumUpdate), repmat(0.001,1,SourceNumUpdate), zeros(1,SourceNumUpdate),zeros(1,SourceNumUpdate)]';
    UpperBoundNonLinear = [S_max*ones(1,SourceNumUpdate),  repmat(0.999,1,SourceNumUpdate), repmat(pi,1,SourceNumUpdate),repmat(2*pi,1,SourceNumUpdate)]';
    problem = createOptimProblem('fmincon','x0',InitialSolution,'ub',UpperBoundNonLinear,'lb',LowerBoundNonLinear, ...
        'objective',@(X) ObjectiveFuncNonLinearLasso(X,Data.Measurement,Lambda, Mesh,F_L2Norm),...
        'options', Opts);
    ...
    RS = Solution.Location(1:SourceNumUpdate);
    THS = Solution.Location(SourceNumUpdate+1:2*SourceNumUpdate);
    PSIS = Solution.Location(2*SourceNumUpdate+1:3*SourceNumUpdate);
